...
%             2. EPISODIC FIXED-LENGTH: % the one that I am recording in pClampex now
...
numOfParams=size(s,1); % get dimensions, now of rows in this instance
clear tmp headPar; % removes tmp and headpar from memory
...
for g=1:numOfParams,
  if fseek(fid, s(g).offs,'bof')~=0, % moving the file pointer to the right place in the file
    fclose(fid);% close the file if the file pointer couldn’t be moved to the specified location
    error(['something went wrong locating ' s(g).name]); 
  end;
  sz=length(s(g).value); %gets the length of the object or dimension in case of an array.
% length of a vector is 1, and for the rest it is the dimension.
...
recChIdx=nADCSamplingSeq(1:nADCNumChannels); % recChIdx <- c(1,3)
% the corresponding indices into loaded data d
recChInd=1:length(recChIdx); % recChInd <- c(1,2)
% the channel names, e.g. 'IN 8' % the name array is divided to 16 elements, 10 char apart
recChNames=(reshape(char(sADCChannelName),10,16))'; 
recChNames=recChNames(recChIdx+1,:); % getting the names of the ADC channels recorded.

chInd=[];
eflag=0;
if ischar(channels) % checks is a single character
  ...
  for i=1:length(channels)% checks if the channel names provided are same as read from the file
...
if fFileVersionNumber>=1.65 % pclamp9 records data in 1.83 fileversion
  addGain=nTelegraphEnable.*fTelegraphAdditGain;%element-to-element multiplication of two arrays
  addGain(addGain==0)=1; % values of addGain which are ‘zero’ are turned to ‘one’
else
  addGain=ones(size(fTelegraphAdditGain)); %creates an array on length(fTele…) of ‘ones’
end

% tell me where the data start
blockSz=512;
switch nDataFormat
  case 0 % Clampex9 records data in this format (int 16)
...
headOffset=lDataSectionPtr*blockSz+nNumPointsIgnored*dataSz; % = (16*512)+(0*2) = 8192
% fADCSampleInterval is the TOTAL sampling interval % total sampling interval  = 10 microsec
si=fADCSampleInterval*nADCNumChannels; % = 10*2

if ischar(sweeps) && sweeps=='a'
  nSweeps=lActualEpisodes; % number of sweeps recorded in the file, nSweeps = 1
  sweeps=1:lActualEpisodes; % sweeps = c(1)
else
...
switch nOperationMode
  ...
  case {2,5}
    if nOperationMode==2 % IMPORTANT 
% this is the mode that is of interest to me now: even-driven fixed-length
      if verbose
    ...
    startPt=0;
    dataPts=lActualAcqLength; % = 1014000
    dataPtsPerChan=dataPts/nADCNumChannels; % = 507000
    if rem(dataPts,nADCNumChannels)>0, 
% checking for divisibility of actualdatalength with no.of ADC channels
      fclose(fid);
      error('number of data points not OK'); 
    end
% checking for divisibility of datalength/channel with no.of sweeps recorded
    dataPtsPerChanPerSweep=dataPtsPerChan/lActualEpisodes;% = 507000 for one sweep 
    if rem(dataPtsPerChan,lActualEpisodes)>0
      fclose(fid);
      error('number of data points not OK');
    end
    dataPtsPerSweep=dataPtsPerChanPerSweep*nADCNumChannels; % 507000 * 2 = 1014000
	% Position the file pointer to the right place on the file to read data.
    if fseek(fid,startPt*dataSz+headOffset,'bof')~=0 % fseek(fid,0*2+8192)
      fclose(fid);
      error('something went wrong positioning file pointer (too few data points ?)'); 
    end
	% Create a 3d matrix of “zeros” to hold the data
    d=zeros(dataPtsPerChanPerSweep,length(chInd),nSweeps); d = zeros(507000, 2, 1)
    % the starting ticks of episodes in sample points WITHIN THE DATA FILE
	% get the position of the beginning of the first sweep to be read
    selectedSegStartInPts=((sweeps-1)*dataPtsPerSweep)*dataSz+headOffset;
	% selectedsegstartInpts = ((1-1)*507000)*2+8192 = 8192
	% move the file pointer to the beginning of the first sweep to be read
    for i=1:nSweeps,
